    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="page-title mb-0 font-size-18">ثبت زیر دسته برای "{{$category->name}}"</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">داشبرد</a></li>
                        <li class="breadcrumb-item"><a href="{{route('admin.category.index')}}">دسته بندی</a></li>
                        @foreach($parents as $parent)
                            <li class="breadcrumb-item"><a href="{{route('admin.sub-category-index',$parent)}}">{{$parent->name}}</a></li>
                    ...
                    <div class="card-body" data-select2-id="9">
                        ...
                        <form method="post" action="{{route('admin.category.update',$edit_item->id)}}" enctype='multipart/form-data' role="form">
                            ...
                            <div class="form-group mb-0">
                                <button  class="btn btn-info waves-effect waves-light">ویرایش</button>
                                <a href="{{route('admin.category.index')}}" class="btn btn-warning waves-effect waves-light">انصراف</a>
                            </div>
            ...
            <div class="col-lg-12">
                <div class="card" data-select2-id="10">
                    <div class="card-body" data-select2-id="9">
                        ...
                        <h4 class="card-title">ثبت زیر دسته -  {{$category->name}}</h4>
                        <form method="post" action="{{route('admin.sub-category-store',$category)}}" enctype='multipart/form-data' role="form">
                            {{csrf_field()}}
                            <div class="form-group">
                                <label class="control-label">زبان</label>
                                <select name="language_id" class="form-control" disabled>
                                    @foreach($languages as $language)
                                        <option @if($category->language_id == $language->id) selected @endif value="{{$language->id}}">{{$language->name}}</option>
                                    @endforeach
                                </select>
                            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="b_datatable" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tbody>
                        @foreach($category->subCategories()->orderBy('sort')->get() as $item)
                            <tr>
                                <td>{{$item->sort}}</td>
                                <td>{{$item->id}}</td>
                                ...
                                <td>
                                    ...
                                    <form class="form-horizontal deleteConfirm" method="POST"
                                          action="{{route('admin.category.destroy',$item)}}" role="form">
                                        ...
                                        <a href="{{route('admin.category.edit',$item)}}" class="">
